...
public class ReportePdfServiceImpl implements ReportePdfService {
    ...
    private static final Color BLUE = new Color(93, 135, 255);
    private static final Color GRAY = new Color(100, 100, 100);
    private static final Color ALT_ROW = new Color(245, 247, 250);

    private Font titleFont() { return FontFactory.getFont(FontFactory.HELVETICA_BOLD, 14); }
    private Font subtitleFont() { return FontFactory.getFont(FontFactory.HELVETICA, 9, Font.NORMAL, GRAY); }
    private Font headerFont() { return FontFactory.getFont(FontFactory.HELVETICA_BOLD, 8, Font.NORMAL, Color.WHITE); }
    private Font cellFont() { return FontFactory.getFont(FontFactory.HELVETICA, 7); }
    private Font boldFont() { return FontFactory.getFont(FontFactory.HELVETICA_BOLD, 8); }

    private void addHeader(Document doc, String titulo, String subtitulo) throws DocumentException {
    ...
    private void addCellAlt(PdfPTable table, String text, Font font, int align, boolean alt) {
        PdfPCell cell = new PdfPCell(new Phrase(text, font));
        cell.setPadding(3);
        cell.setHorizontalAlignment(align);
    ...
    public byte[] generarLibroDiario(List<AsientoResponse> asientos, LocalDate desde, LocalDate hasta) {
        return buildPdf(doc -> {
            ...
            PdfPTable table = createTable(new String[]{"#", "Fecha", "Descripcion", "Debito", "Credito"}, new float[]{1, 2, 6, 2, 2});
            ...
            doc.add(new Paragraph("Total Debito: " + MONEY.format(totalD) + "  |  Total Credito: " + MONEY.format(totalC) + "  |  Asientos: " + asientos.size(), boldFont()));
        }).toByteArray();
    }
    ...
    public byte[] generarLibroMayor(LibroMayorResponse data) {
        return buildPdf(doc -> {
            ...
            PdfPTable table = createTable(new String[]{"Cuenta", "Nombre", "Debito", "Credito"}, new float[]{2, 5, 2, 2});
            ...
            doc.add(table);
            doc.add(new Paragraph("Total Debitos: " + MONEY.format(data.totalDebitos()) +
                    "  |  Total Creditos: " + MONEY.format(data.totalCreditos()) +
    ...
    public byte[] generarBalanceGeneral(BalanceGeneralResponse data) {
        return buildPdf(doc -> {
            ...
            doc.add(table);
            doc.add(new Paragraph(data.cuadrado() ? "Activos = Pasivos + Patrimonio" : "Balance descuadrado", boldFont()));
        }).toByteArray();
    }
    ...
    public byte[] generarNomina(List<NominaResponse> nominas) {
        return buildPdf(doc -> {
            addHeader(doc, "Nomina", nominas.size() + " registro(s)");
            PdfPTable table = createTable(new String[]{"Empleado", "Periodo", "Salario Base", "Deducciones", "Neto", "Estado"}, new float[]{3, 4, 2, 2, 2, 1});
